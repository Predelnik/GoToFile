...
public:
  ...
  reference at(EnumT index) { return parent_t::at(static_cast<size_t>(index)); }
  ...
  template <EnumT Index> friend ValueT &get(self &value) {
    using std::get;
  ...
  template <EnumT Index> friend const ValueT &get(const self &value) {
    using std::get;
